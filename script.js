  ...
  clues.forEach((clue) => {
    ...
    for (let i = 0; i < word.length; i++) {
      ...
      if (existing !== "#" && existing !== ch) {
        console.warn(`Конфликт букв для слова ${clue.number} в клетке (${r},${c}): было "${existing}", ставим "${ch}"`);
      }
      grid[r][c] = ch;
    }
  });
...
// -------------------------
// 3. ЭЛЕМЕНТЫ СТРАНИЦЫ (ВАЖНО: id как в твоём HTML)
// -------------------------
...
function addClueNumbers() {
  clues.forEach((clue) => {
    ...
    // Привязка номера слова ко всем клеткам слова
    for (let i = 0; i < word.length; i++) {
...
function highlightClueForCell(cell) {
  ...
  const prev = gridElement.querySelectorAll(".puzzle-cell.active-word");
  prev.forEach((c) => c.classList.remove("active-word"));
...
// -------------------------
// 8. ВЫВОД ВОПРОСОВ (ВНИЗУ В ОДИН СПИСОК)
// -------------------------
...
function hintActiveWord() {
  ...
  if (!activeClueNumber) {
    resultMessage.textContent = "Выберите слово: нажмите на вопрос внизу или на клетку слова.";
    return;
  }
